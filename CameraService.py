...
def send_video_streamFP(origin):
    #Inspection line video streamaing modification
    global sending_video_streamFP
    global cap
    global external_client

    topic_to_publish = f"cameraService/{origin}/videoFrameFP"

    while sending_video_streamFP:
        # Read Frame
        ret, frame = cap.read()
        if ret:
            _, image_buffer = cv.imencode(".jpg", frame)
            jpg_as_text = base64.b64encode(image_buffer)
            external_client.publish(topic_to_publish, jpg_as_text)
            time.sleep(10)
...
def process_message(message, client):
    ...
    if command == "takePictureFP":
        print("Take picture of TT")
        ret = False
        for n in range(1, 20):
            # this loop is required to discard first frames
            ret, frame = cap.read()
        ...
        # Converting into encoded bytes
        jpg_as_text = base64.b64encode(image_buffer)
        external_client.publish("cameraService/" + origin + "/pictureFP", jpg_as_text)

    if command == "shutCamOFF":
        print("Shut off camera")
        cap.release()
    ...
    if command == "startVideoStreamFP":
        print("start video stream for TT")
        sending_video_streamFP = True
        w = threading.Thread(target=send_video_streamFP,args=[origin] )
        w.start()

    if command == "stopVideoStreamFP":
        print("stop video stream for TT")
        sending_video_streamFP = False
        external_client.publish("cameraService/"+origin+"/cameraOFF")
